...
export function App({ client = apiClient }: { client?: ApiClient }) {
  ...
  const openAgentCatalog = () => { setSurface("agents"); const projectQuery = selectedProject ? `?project_id=${encodeURIComponent(selectedProject)}` : ""; window.history.pushState({}, "", `/agents${projectQuery}`); };

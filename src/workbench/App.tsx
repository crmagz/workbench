...
function logSyntax(content: string) {
  const tokenPattern = /("(?:\\.|[^"\\])*")(?=\s*:)|(\b(?:ERROR|FATAL|WARN|WARNING|INFO|DEBUG|SUCCESS|FAILED|SUCCEEDED)\b)|\b(true|false|null)\b|(-?(?:0|[1-9]\d*)(?:\.\d+)?(?:[eE][+-]?\d+)?)/gi;
  const tokens = [];
  let lastIndex = 0;
  for (let match = tokenPattern.exec(content); match; match = tokenPattern.exec(content)) {
    if (match.index > lastIndex) tokens.push(content.slice(lastIndex, match.index));
    const tone = match[1] ? "log-key" : match[2] ? `log-level ${match[2].toLowerCase()}` : match[3] ? "json-literal" : "json-number";
    tokens.push(<span className={tone} key={`${match.index}:${match[0]}`}>{match[0]}</span>);
    lastIndex = match.index + match[0].length;
  }
  if (lastIndex < content.length) tokens.push(content.slice(lastIndex));
...
function auditEventState(event: TimelineEvent) {
  ...
  return event.delivered ? "delivered" : "recorded";
}
...
function Timeline({ client = apiClient, runId = "", events, title = "Authoritative timeline", failureSummary }: { client?: ApiClient; runId?: string; events: TimelineEvent[]; title?: string; failureSummary?: string | null }) {
  ...
  useEffect(() => { setExpanded(new Set()); setLogs({}); setLoading(new Set()); setErrors({}); setFailedCursors({}); }, [runId]);
  const loadLogs = async (event: TimelineEvent, cursor?: string) => {
    ...
    try {
      const page = await client.getAuditLogs(runId, event.event_id, cursor);
      setLogs((current) => {
        const previous = current[event.event_id];
        return { ...current, [event.event_id]: cursor && previous ? appendAuditLogPage(previous, page) : page };
      });
    } catch {
  ...
  return <section className="card audit-timeline" style={{ "--audit-status-column": auditStatusColumn } as CSSProperties}><h2 className="panel-title">{title}</h2>{events.length === 0 ? <p className="control-note">No persisted lifecycle events are available yet.</p> : <div className="timeline"><div className="audit-column-headings" aria-hidden="true"><span>Status</span><span>Type</span><span>Activity</span><span>Audit event ID</span><span>Occurred</span><span>Evidence</span></div>{events.map((event) => {
    ...
    return <div className="audit-event" key={event.event_id}><div className="audit-event-row"><span className="flow-status"><Pill status={state} /></span><span className={`audit-activity-kind ${auditActivityKind(event)}`}>{auditActivityLabel(event)}</span><span className="audit-event-primary"><b>{statusLabel(event.event_type)}</b><small>{auditEventContext(event)}</small>{failureReason && <p className="audit-failure" role="alert"><b>Failure reason:</b> {failureReason}</p>}</span><span className="audit-event-identity"><small>Audit event ID</small><b>{event.event_id}</b></span><span className="audit-event-occurred"><small>Occurred at</small><b>{new Date(event.occurred_at).toLocaleString()}</b></span>{event.log_evidence_available ? <button className="audit-expand" type="button" onClick={() => toggle(event)} aria-expanded={isExpanded} aria-controls={detailId}>{isExpanded ? "Hide logs" : "View logs →"}</button> : <span className="audit-evidence-unavailable">{auditEvidenceLabel(event)}</span>}</div>{isExpanded && <div className="audit-details" id={detailId}><div className="audit-output" aria-busy={isLoading}>{isLoading && <p className="control-note" role="status">Loading correlated output…</p>}{errors[event.event_id] && <p className="evidence-error" role="alert">{errors[event.event_id]} <button className="text-button" type="button" onClick={() => void loadLogs(event, failedCursors[event.event_id])}>Retry</button></p>}{logPage && <>{logPage.lines.length > 0 ? <pre className="audit-log-output" aria-label="Redacted correlated log output"><code>{logPage.lines.map((line) => <span className="audit-log-line" key={auditLogLineKey(line)}><time className="audit-log-time" dateTime={line.timestamp}>{line.timestamp}</time><span className="audit-log-stream">[{line.stream}]</span><span className="audit-log-message">{logSyntax(line.message)}</span></span>)}</code></pre> : <p className="control-note">{logAvailabilityMessage(logPage.availability)}</p>}{logPage.next_cursor && <button className="text-button" type="button" disabled={isLoading} onClick={() => void loadLogs(event, logPage.next_cursor!)}>Load earlier output</button>}</>}</div></div>}</div>;
  })}</div>}</section>;
}

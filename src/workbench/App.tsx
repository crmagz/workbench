...
function SpecificationEvidencePanes({ client, run, artifacts, initial }: { client: ApiClient; run: Run; artifacts: Artifact[]; initial?: Artifact["kind"] }) {
  ...
  return <div className="specification-evidence-panes" aria-label={`${artifactLabel(artifacts[0].kind)} main artifact panel`}>{artifacts.map((artifact) => <SpecificationEvidencePane key={`${artifact.kind}:${artifact.sha256}`} artifact={artifact} content={content[artifact.sha256]} loaded={Object.hasOwn(content, artifact.sha256)} revision={run.product_specification_revision} />)}{error && <p className="evidence-error" role="alert">{error}</p>}</div>;
}

function SpecificationEvidencePane({ artifact, content, loaded, revision }: { artifact: Artifact; content?: string; loaded: boolean; revision?: number | null }) {
  const [format, setFormat] = useState<"yaml" | "json">("yaml");
  const label = artifact.kind === "plan" ? "Plan" : artifactLabel(artifact.kind);

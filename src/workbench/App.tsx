...
export function App({ client = apiClient }: { client?: ApiClient }) {
  ...
  const refreshSelected = useCallback(async (): Promise<boolean> => { if (!selectedRun) return false; try { const detail = await client.getRun(selectedRun.run_id); setSelected((previous) => previous?.run_id === detail.run_id ? detail : previous); void refreshTimeline(selectedRun.run_id); return true; } catch { setError("Unable to load the latest scoped run detail; showing the last verified summary."); return false; } }, [client, refreshTimeline, selectedRun]);
